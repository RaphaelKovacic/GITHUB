...
public class AnalysisAgent extends Agent{
	...
	class SubBehaviour extends OneShotBehaviour{
		public void action(){
			// Si l'agent n'est pas occup� il faut envoy� un message Subscribe � l'agent de Simu
			if(isOccuped == false){
				...
				message1.setContent("Je suis pr�t � travailler");
				myAgent.send(message1);

				// L'agent passe en occup�;
				isOccuped = true;
				}else{
					System.out.println(
	...
	class TraiteCellsBehaviour extends OneShotBehaviour{
		...
		public void action() {
			...
			// On execute les diff�rents algo... (modifie cells en cons�quence)
			...
			while (cells_modif == true){
				...
				for(int i = 0; i < cells.size(); i++)
				{
					...
					// Si la cellule actuelle a deja une valeur d�fini (2)
					...
					if (cells.get(i).getLPossibles().size() == 2){
						...
						for(int j = i+1; j < cells.size(); j++){
							if (cells.get(j).getLPossibles().size() == 2){
								...
								// Si elle contient les 2 m�me valeurs...
				...
				for(int i = 0; i < LPossiblesTot.size(); i++){
					...
					// Si on a trouv�e qu'une seule fois la valeur possible parmis toutes les ListePossible
					if (occur_VP == 1){
