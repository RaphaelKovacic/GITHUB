...
public class SimulationAgent extends Agent{
		...
		addBehaviour(new TickerBehaviour(this, 5000){
			protected void onTick() {
				if (SudokuFinished == false){
					...
					// Si on a recu les 27 subrscribe, on parrall�lise les traitements.
					if (nb_subs_recu == 27){
						for (int i = 0 ; i < 27 ; i++){
							NameOfAna = "Analyse"+i;
					...
					// Sinon on demande au DF de distribuer les messsages al�atoirement � des agents Analyse...
					else{
						for (int i = 0 ; i < 27 ; i++){
					...
					// Sinon si le Sudoku est termin� on Kill la simulation.
				}else{
					doDelete();
				}
			}
		});
		//Behaviour qui s'occupe de r�ceptionner les 'Subscribe' des agents d'analyses.
		addBehaviour(new WaitBehaviour());

		//Behaviour qui s'occupe de r�ceptionner les 'Inform' de l'agent d Environnemnt.
		addBehaviour(new WaitEnvBehaviour());


	} // fin Setup

	// Fonction permettant de trouver un agent mult (choisit al�atoirement)
	...
	class WaitBehaviour extends CyclicBehaviour{
		...
		@Override
		public void action() {
			// On attend la r�ception de message de type Subscribe
	...
	class WaitEnvBehaviour extends CyclicBehaviour{
		...
		@Override
		public void action() {
			// On attend la r�ception de message de type Inform
	...
	class TraiteRepAnalyse extends SimpleBehaviour{
		...
		public void action() {
			...
			if (message != null){
				// On r�cup�re le message et on met � jour notre copie de sudoku.
				...
				// Si jamais on a re�u les 27 messages :
				if (nb_rep == 27){
					nb_rep = 0;
					endOf = true;
					// On envoie notre Sudoku "Copy" � Env pour qu'il fasse la Maj du "Vrai" Sudoku.
					myAgent.addBehaviour(new TraiteMajBehaviour());
				}

			}else{
				// Sinon on attend l'arriv� d'un message
				block();		
			}
		}
	}
